...
iptables -A INPUT -p tcp --dport 80 -i enp0s3 -m state --state NEW -m recent --set

iptables -I INPUT -p tcp --dport 80 -m state --state NEW -m recent --update --seconds 10 --hitcount 20 -j DROP

iptables -A INPUT -p tcp --dport 443 -i enp0s3 -m state --state NEW -m recent --set
...
iptables -t filter -A INPUT -i enp0s3 -p tcp --syn -j syn-flood
